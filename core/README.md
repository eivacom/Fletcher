...
| `kReentrantCall` | 10 | The caller re-entered the seam from inside a delivery callback, on the same provider instance and the same thread, through a door that cannot serve it there. Every seam method — the four data-path methods and the two schema-only ones — refuses, on every provider (spec §6 clause 6). Distinct from `kNotSupported`, which says the provider cannot do this at all. |
