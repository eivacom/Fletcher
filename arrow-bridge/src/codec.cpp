...
void EncodePositionalValue(std::vector<uint8_t>& buf,
    ...
    case T::DENSE_UNION: {
        ...
        if (child_id < 0)
            throw std::invalid_argument("Codec: invalid dense union type_code");
        ...
        if (ReadNullBit(bitfield, static_cast<int>(i))) {
            ...
            if (!st.ok())
                throw std::invalid_argument(
                    "Codec: builder AppendNull failed: " + st.ToString());
        } else {
            auto scalar = DecodePositionalValue(r, elem_type);
            auto st = builder->AppendScalar(*scalar);
            if (!st.ok())
                throw std::invalid_argument(
                    "Codec: builder AppendScalar failed: " + st.ToString());
        }
    }
...
std::shared_ptr<arrow::Scalar> DecodePositionalValue(
    ...
    case T::MAP: {
        ...
        for (uint32_t i = 0; i < count; ++i) {
            ...
            if (!st.ok())
                throw std::invalid_argument(
                    "Codec: key append failed: " + st.ToString());
        }
    ...
    case T::SPARSE_UNION: {
        ...
        if (child_id < 0)
            throw std::invalid_argument(
                "Codec: unknown sparse union type_code");
    ...
    case T::DENSE_UNION: {
        ...
        if (child_id < 0)
            throw std::invalid_argument(
                "Codec: unknown dense union type_code");
...
Codec::Codec(std::shared_ptr<arrow::Schema> schema)
...
EncodedRow Codec::EncodeRow(const ArrowRow& values) const {
    ...
    for (int i = 0; i < num_fields; ++i) {
        ...
        if (scalar->type->id() != schema_->field(i)->type()->id())
            throw std::invalid_argument(
                "Codec::EncodeRow: type mismatch for field '" +
...
ArrowRow Codec::DecodeRow(const EncodedRow& buf) const {
