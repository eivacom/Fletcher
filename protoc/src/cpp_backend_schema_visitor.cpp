...
void CppSchemaSink::DeepCopyMessageStruct(const google::protobuf::Descriptor* nested_msg,
                                          SchemaRef dst, const OptionMetadataResolver* resolver) {
...
void NanoarrowSchemaSink::SetTypeDateTime(SchemaRef schema, ArrowType type, ArrowTimeUnit time_unit,
                                          const char* timezone) {
    CheckNa(
        ArrowSchemaSetTypeDateTime(static_cast<ArrowSchema*>(schema), type, time_unit, timezone),
        "set datetime type");
}
...
void NanoarrowSchemaSink::SetMetadata(
    SchemaRef schema, const std::vector<std::pair<std::string, std::string>>& pairs) {
    auto* s = static_cast<ArrowSchema*>(schema);
    ArrowBuffer buf;
    ArrowBufferInit(&buf);
...
void NanoarrowSchemaSink::DeepCopyMessageStruct(const google::protobuf::Descriptor* nested_msg,
    ...
    CheckNa(ArrowSchemaDeepCopy(nested.get(), static_cast<ArrowSchema*>(dst)),
            "copy struct schema");
}
...
SchemaVisitor::MetaPairs SchemaVisitor::FieldMetadata(const SchemaFieldRecord& f) const {
    MetaPairs pairs = {{"field_number", std::to_string(f.field_number)}, {"field_id", f.field_id}};
    if (resolver_ != nullptr) {
