...
| Item | Detail | Source |
|---|---|---|
| ROUND-1 | **The conformance harness cannot tell whose Agent it is talking to.** `SpawnedAgentAlive()` (`integration-tests/pubsub-conformance`, landed by PDA-DEC-1 in `a963211`) accepts an Agent it did not start, so a MicroXRCEAgent left running from an earlier session serves the whole run and the harness reports `conformance_xrce` 25/25 PASSED. Proved by the PDA-DEC-7 cycle-2 re-reviewer, which ran the suite green against a deliberately foreign Agent. Consequence: every XRCE green since PDA-DEC-1 is conditional on "no stray Agent was listening", and the round's own false-green discipline does not cover it. **PDA-DEC-1 is closed, so no item owns this.** Recommendation: fix in-round before PDA-DEC-9 signs the handoff, since both ABI rounds inherit this harness as the conformance oracle. Cheapest form: have the harness prove ownership of the Agent process it spawned (pid check) rather than probing a port. | PDA-DEC-7 compliance cycle 2 |
