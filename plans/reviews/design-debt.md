...
### Cycle 2 (design revision 2, `843cdb0`) — all ten non-blocking

Cycle-1 AG1-DEBT-1 is **retired**: the one-argument `TranslateSeamFailure` is unchanged, so
`provider_registry.cpp:186,211` and `core/tests/test_write_buffer.cpp:453` need no edit.
AG1-DEBT-2 → `AbsorbedCount()`; -3 → the lift; -4, -5, -6, -7, -8 all addressed in the text.
