...
| PDA-DEC-A4 | §7 clause 6 vs the tier §9 assigns to BIND — **the only memory-safety defect**, and first by severity. Scoped to also carry the *Unsubscribe idempotence* disagreement verification found: the seam documents it as "a no-op, not an error, safe to call unconditionally on teardown" while `subscriber.cpp` throws `kInvalidArgument` for an unknown id — the one call a C# finalizer makes unconditionally, and a finalizer cannot let an exception escape. | 🟥 | TBD at design | ⚪ |
| PDA-DEC-A5 | §3.5 — an embedded NUL truncates a topic name **on the wire**; verification found the *participant* name truncates too. **The only confirmed silent wrong answer already in the tree.** Scoped to also carry the embedded-`/` finding: `{"a/b"}` and `{"a","b"}` are one topic in all three providers while §3.5's frozen sentence licenses a provider under which they are two. | 🟥 | TBD at design | ⚪ |
| PDA-DEC-A1 | §3.1 — the C form the spec describes is not constructible from `WriteBuffer`'s public API for a binding that is *handed* one: `Data()` is `const`, there is no `Capacity()` anywhere, and every `Append*` copies from a source. Worse than claimed: breaks frozen §8, and `CopyAccounting` is structurally blind to the binding-side copy. Note `VectorWriteBuffer` already has a private `Reserve` that a public one would shadow — a naming call to make with the design. | 🟧 | TBD at design | ⚪ |
| PDA-DEC-A2 | §5.3 — frozen as an unmet obligation ("the seam *must state*…") that never reached a header. Three providers, three behaviours; a subscriber's exception can surface at an unrelated publisher's `Publish` as **`kPayloadTooLarge`**, because §5.1 maps by type unconditionally. Worse than claimed: XRCE's own comments call the path UB, and there is a second wrong-party path via `CreateTopic`'s translator. | 🟧 | TBD at design | ⚪ |
| PDA-DEC-A3 | §6 — no re-entrancy clause; three providers behave three ways, and Fast DDS is a third *undocumented* answer. Carries **`kReentrantCall = 10`**, the append the owner allocated 2026-09-03. | 🟧 | TBD at design | ⚪ |
| PDA-DEC-A6 | §3.2 — `Blob` has a C form; the `Attachments` container does not. Blocks PDA-ABI equally: the ABI spec has no attachments type either. | 🟦 | TBD at design | ⚪ |
| PDA-DEC-A7 | §5.1 — no C form for the error *message*, which for §4 configuration refusals **is** the answer (it names the offending key). The ABI spec already has the rule the seam lacks. | 🟦 | TBD at design | ⚪ |
| PDA-DEC-A8 | The append itself: `kReentrantCall = 10` across the enum, the `static_assert` set, `core/README.md`'s published table and the taxonomy guard — which makes an un-published append **fail to compile**, so it enforces the ruling rather than documenting it. May land with A3. | 🟦 | `Taxonomy.PublishedNumbersMatchTheEnum` extended | ⚪ |
