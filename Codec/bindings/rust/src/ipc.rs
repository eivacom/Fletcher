...
pub(crate) fn serialize_batch(batch: &RecordBatch) -> Result<Vec<u8>, Error> {
    let mut buf = Vec::new();
    let mut w = StreamWriter::try_new(&mut buf, batch.schema_ref())
        .map_err(|e| Error(e.to_string()))?;
    w.write(batch).map_err(|e| Error(e.to_string()))?;
    w.finish().map_err(|e| Error(e.to_string()))?;
    Ok(buf)
}
