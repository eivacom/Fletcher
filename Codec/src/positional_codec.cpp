...
void WriteNullBitfield(std::vector<uint8_t>& buf,
    ...
    for (int i = 0; i < count; ++i) {
        ...
        if (is_null)
            buf[start + i / 8] |= static_cast<uint8_t>(1u << (i % 8));
    }
}
...
void EncodePositionalValue(std::vector<uint8_t>& buf,
                           const arrow::Scalar& scalar,
                           const arrow::DataType& type);
std::shared_ptr<arrow::Scalar> DecodePositionalValue(
    detail::Reader& r,
...
void EncodeListElements(std::vector<uint8_t>& buf,
...
void EncodePositionalValue(std::vector<uint8_t>& buf,
    ...
    case T::SPARSE_UNION: {
        ...
        if (child_id < 0 || child_id >= static_cast<int>(us.value.size()))
            throw std::invalid_argument("PositionalCodec: invalid sparse union type_code");
    ...
    case T::DENSE_UNION: {
        ...
        if (child_id < 0)
            throw std::invalid_argument("PositionalCodec: invalid dense union type_code");
        EncodePositionalValue(buf, *us.value,
                              *union_type.field(child_id)->type());
        return;
    }
    default:
        ...
        if (ReadNullBit(bitfield, static_cast<int>(i))) {
            ...
            if (!st.ok())
                throw std::invalid_argument(
                    "PositionalCodec: builder AppendNull failed: " + st.ToString());
        } else {
            auto scalar = DecodePositionalValue(r, elem_type);
            auto st = builder->AppendScalar(*scalar);
            if (!st.ok())
                throw std::invalid_argument(
                    "PositionalCodec: builder AppendScalar failed: " + st.ToString());
        }
    }
...
std::shared_ptr<arrow::Scalar> DecodePositionalValue(
    ...
    case T::MAP: {
        ...
        for (uint32_t i = 0; i < count; ++i) {
            ...
            if (!st.ok())
                throw std::invalid_argument(
                    "PositionalCodec: key append failed: " + st.ToString());
        }
        ...
        for (uint32_t i = 0; i < count; ++i) {
            if (ReadNullBit(val_bitfield, static_cast<int>(i))) {
                ...
                if (!st.ok())
                    throw std::invalid_argument(
                        "PositionalCodec: value AppendNull failed: " + st.ToString());
            } else {
                auto val = DecodePositionalValue(r, map_type.item_type());
                auto st = val_builder->AppendScalar(*val);
                if (!st.ok())
                    throw std::invalid_argument(
                        "PositionalCodec: value append failed: " + st.ToString());
            }
        }
    ...
    case T::SPARSE_UNION: {
        ...
        if (child_id < 0)
            throw std::invalid_argument(
                "PositionalCodec: unknown sparse union type_code");
    ...
    case T::DENSE_UNION: {
        ...
        if (child_id < 0)
            throw std::invalid_argument(
                "PositionalCodec: unknown dense union type_code");
        return std::make_shared<arrow::DenseUnionScalar>(
            DecodePositionalValue(r, union_type.field(child_id)->type()),
            type_code, type);
    }
    default:
...
PositionalCodec::PositionalCodec(std::shared_ptr<arrow::Schema> schema)
    : schema_(std::move(schema)) {}

EncodedRow PositionalCodec::EncodeRow(const ArrowRow& values) const {
    const int num_fields = schema_->num_fields();

    if (static_cast<int>(values.size()) != num_fields)
        throw std::invalid_argument(
            "PositionalCodec::EncodeRow: values.size() (" +
            std::to_string(values.size()) +
            ") does not match schema.num_fields() (" +
    ...
    for (int i = 0; i < num_fields; ++i) {
        ...
        if (scalar->type->id() != schema_->field(i)->type()->id())
            throw std::invalid_argument(
                "PositionalCodec::EncodeRow: type mismatch for field '" +
...
ArrowRow PositionalCodec::DecodeRow(const EncodedRow& buf) const {
    return DecodeRow(buf.data(), buf.size());
}

ArrowRow PositionalCodec::DecodeRow(const uint8_t* data, size_t len) const {
