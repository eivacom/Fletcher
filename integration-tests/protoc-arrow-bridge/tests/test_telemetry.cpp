...
ArrowRow MakeArrowRow(int32_t sensor_id, double temperature, const std::string &label, bool valid,
...
TEST(ProtocArrowBridgeByteCompat, GeneratedEncodeMatchesCodecEncode) {
    ...
    EncodedRow codec_bytes =
        codec.EncodeRow(MakeArrowRow(42, 23.5, "intake", true, {100, 200, 300}));
...
TEST(ProtocArrowBridgeByteCompat, GeneratedEncodingDecodesViaCodec) {
    fletcher_gen::integration::Telemetry generated;
    generated.set_sensor_id(7).set_temperature(-12.75).set_label("").set_valid(false).set_readings(
        {10, 20, 30});
...
TEST(ProtocArrowBridgeByteCompat, CodecEncodingDecodesViaGenerated) {
    Codec codec(TelemetrySchema());
    EncodedRow codec_bytes = codec.EncodeRow(MakeArrowRow(99, 6.25, "cabin", true, {1, 2, 3}));
