...
1. **Local plugin build**: `conan create core` + `conan create protoc` produces the native `fletcher-protoc` binary.
...
3. **Local-path npm install**: `npm install ../../protoc/npm` installs `@eiva/protoc-gen-fletcher` from this working copy. npm auto-installs the `@protobuf-ts/protoc` peerDependency from the public registry.
...
See the repo root's [Development environment](../../README.md#development-environment) section for how to open the devcontainer. From the repo root inside the container:

### 1. Build the protoc plugin into the local Conan cache
...
BIN=$(find ~/.conan2/p/b -path '*/fletcher-protoc/*/p/bin/fletcher-protoc' -type f | head -1)
...
```bash
npm install --save-dev ../../protoc/npm @eiva/fletcher-gateway-client
```
