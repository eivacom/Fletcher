...
## HARD-7 — Consolidate duplicated helpers (2026-07-10)

**Forcing test:** existing suites green + single-definition check → 🟢   (refactor — behaviour-preserving; no new red-green test)
**Design:** `plans/HARD-7-consolidate-helpers.md`  ·  Step-2: APPROVE (cycle 2)
**What landed:** Three duplicated helpers consolidated to one source each, byte-identical (H-INV-1 / decision #6): `JoinSegments` → existing `pubsub/.../internal/segments.hpp` (deleted copies in xrce provider, pubsub-arrow publisher/subscriber, and the `Join` mocks in 2 tests + gateway; re-pointed call sites); `AppendFixed<T>` → single `inline` template in the existing private `arrow-bridge/src/scalar_codec.hpp` (both anon-namespace copies deleted; no new public header); `BitfieldBytes` → new `core/include/fletcher/core/detail/bitfield.hpp` (`inline size_t BitfieldBytes(int64_t n)`; codec's int64_t copy + both positional_io size_t copies collapsed; int64_t overflow-safe intent preserved via explicit `static_cast<int64_t>` on uint32_t counts). Bonus: `MapContext::BeginValues`' inline `(count+7)/8` folded onto the shared helper. Net −55 lines.
**Files touched:** `arrow-bridge/src/{codec.cpp,scalar_codec.cpp,scalar_codec.hpp}`, `core/include/fletcher/core/detail/bitfield.hpp` (new), `core/include/fletcher/core/positional_io.hpp`, `gateway/src/main.cpp`, `pubsub-arrow/{src,include,tests}` (publisher/subscriber + test mock), `pubsub/tests/test_publisher_subscriber.cpp`, `xrcedds-pubsub-provider/src/xrce_dds_pubsub_provider.cpp`
**Reviews:** compliance `CONFORMS` · code-review `blocking 0 / should-fix 0 / nit 0`
**Verification:** inner-loop `PASS — core 25/25, arrow-bridge 35/35, pubsub 17/17, pubsub-arrow 16/16, xrce 10/10, gateway 20/20`; **ROUND-CLOSE CHECKPOINT 🟢** — all 6 components + integration `72/72`, **0 warnings**, no byte-identity regression (HARD-1…7 combined).
**Commit / push:** `hard/7-consolidate-helpers` → `origin hard/7-consolidate-helpers` (draft PR, base `hard/6-nodiscard`)
**Carry-forwards / stop-and-asks:** none. The optional Windows-only `verify_*.ps1` single-definition gate was intentionally NOT committed (single-definition confirmed by grep); acceptance was the green suites + byte-identity.
