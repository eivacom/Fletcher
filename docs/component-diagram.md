...
graph TB
    ...
    subgraph server_tier ["Server Tier (Arrow C++)"]
        ...
        POS_CODEC["Codec<br/><i>ArrowRow encode/decode</i>"]
...
graph TB
    ...
    CODEC["codec (ArrowBridge)<br/><i>core + Arrow C++, server-side</i>"]

    PUBSUB_ARROW["pubsub_arrow<br/><i>pubsub + codec<br/>+ Arrow C++</i>"]
...
graph TB
    subgraph root ["ArrowRowSerializer/"]
        ...
        subgraph ab_dir ["ArrowBridge/"]
            AB_INC["include/<br/><i>Codec, ArrowRowView,<br/>CRS utils</i>"]
            AB_SRC["src/"]
            AB_TEST["tests/"]
        end
