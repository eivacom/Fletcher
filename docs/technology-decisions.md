...
**Risks:** The positional format requires schema agreement between publisher and subscriber — a schema version mismatch produces corrupt data rather than a clean error. This is mitigated by the schema transport mechanism, but direct byte-level debugging is harder than with self-describing formats. The legacy tagged codec (`RowCodec`) is retained in the codebase for backward compatibility but is no longer used by generated code.
