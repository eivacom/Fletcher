...
**Superseded in part — read TD-008 and the seam spec for the current model.** Since round PDA-decouple (item 3), `SubscriptionResult` carries a **`SchemaArrival`** ([pubsub/include/fletcher/pubsub/provider.hpp](../pubsub/include/fletcher/pubsub/provider.hpp)) — a waitable handle with a typed outcome — not the schema itself. The field is still *named* `schema`, which is what makes the Decision paragraph above read as still-true: `Subscribe()` **never blocks** and does not deliver a schema, the caller waits on the arrival afterwards ([pubsub-interface-spec.md](pubsub-interface-spec.md) §3.4 and §7 clause 5). The retry in the Risks paragraph is gone with it: a provider now holds a persistent companion `__schema` reader and resolves the arrival when the announcement lands, so there is no poll and no timeout. The companion-topic mechanism itself, and everything else in this entry, is unchanged. This entry is kept as the record of the decision as it was taken.
...
## TD-008: The Pub/Sub Seam as a Frozen Contract with Uniform Provider Selection

**Status:** Accepted

**Context:** Two efforts were queued on opposite sides of one seam — a protocol driver ABI *below* `PubSubProvider` (so a protocol becomes a runtime-loadable driver) and C#/Rust language bindings *above* it — and the owner required them to proceed in parallel, meeting only at that interface. As it stood the seam could not carry either: the types crossing it had no stated C-expressible ownership model, failures crossed as C++ exceptions, the delivery contract was prose honoured by review, and *which* provider was in use was a compile-time decision baked into every caller. Two teams mirroring a moving interface weeks apart, without talking, is how they diverge silently. (This premise was originally worded as a *pure C* driver ABI. The owner ruled on 2026-09-05 that every protocol driver is C++ and therefore both sides of that ABI are C++; the phrase is corrected here rather than preserved, because it is the exact wording the ruling was given against — see [protocol-driver-abi-spec.md](protocol-driver-abi-spec.md) §0 and [pubsub-interface-spec.md](pubsub-interface-spec.md) §0. The C *forms* at the boundary, and the rule that no exception may cross it, are unaffected.)

**Decision:** The seam is a single **frozen contract** — [docs/pubsub-interface-spec.md](pubsub-interface-spec.md), whose §12 states what is frozen, who may act on each class of text, and how each handoff condition was verified. Four properties are normative. **Selection** is uniform: one registry turns a *selector* plus a *configuration* into a provider, a selector is a name or a path by its shape alone, and whether the provider is built in or loaded through an ABI is invisible above the seam. **Configuration** is a small typed core (`{max_payload_bytes, domain_id}`) plus an opaque per-protocol document Fletcher transports and never parses — Fast DDS's native XML QoS profile text, `key=value` for XRCE. **Failure** is one error type carrying one stable numbered cause (`PubSubError` over `PubSubStatus`), appended only, published as a table beside the header that defines it. **Ownership** of every crossing type is written normatively in the header, and schema arrival is a waitable handle with typed outcomes rather than a `std::shared_future`. Neither later round may change any of it: finding the seam insufficient is a stop-and-ask against the spec, not a local workaround and not a change landed inside an ABI round.

**Rationale:** Freezing buys parallelism, which was the whole objective. A single numbered cause is what two independent bindings can mirror without drifting, because exceptions cannot cross a C boundary and an assortment of standard exception types plus a prose map cannot be mirrored twice identically. A typed core plus an opaque document keeps Fletcher from learning any protocol's vocabulary while still letting an operator configure one fully. Name-or-path selection by shape means one configuration string means the same thing in every build, and the driver ABI adds only a resolver behind the same `Create` call — pinned by a `static_assert` on that member's whole type. The delivery contract stops being prose: it is a conformance suite run against every provider, with a copy-accounting oracle making zero-copy falsifiable, and both later rounds inherit those suites as their oracle rather than re-deriving the rules.

**Alternatives considered:** Letting each later round amend the seam as it discovered gaps (the fastest start, and the silent-divergence failure this decision exists to prevent). A shared C header or shared buffer struct between the two ABIs (couples their release cadence and forces one round to wait on the other — explicitly rejected by the owner). Defining the binding ABI as pass-through of driver-ABI types (works only when the provider happens to be a loaded driver, and breaks for a built-in one). Keeping protocol-typed configuration structs at the seam (leaves Fletcher knowing one protocol's vocabulary) or having Fletcher parse a config file (a parser and its failure modes for a format only the provider understands). Keeping the `shared_future` schema wait beside the new handle as a C++ convenience (two mechanisms drift, and it leaves the binding path untested because no in-tree caller would exercise it).

**Risks:** The freeze buys parallelism at the price of a stop-and-ask whenever either round finds the seam short — and the round most likely to find it short is the one adding loading. One condition (nothing above the seam branching on built-in versus loaded) is held by the freeze rather than by any machine, because a machine check would have to add the very distinction the decision removes. The contract was frozen on evidence produced by local runs on one Windows machine plus one WSL compile, with no automated build yet run on the branch, so Linux behaviour is unverified and a Linux-only difference is routed to the owner as a question (spec §12.4). Two transport blind spots survive with named owners: the receive-side data-sharing defect and the unbounded-wait clause, which no test in this tree can pin honestly.
