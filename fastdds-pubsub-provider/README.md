...
`LoanableSampleWriter` on the publish side (`internal/sample_writer.hpp`), `CopyingDataReaderListener` /
`LoanedDataReaderListener` on the subscribe side (`internal/data_reader_listener.hpp`), each driven by
...
| `reliable_writer_qos().times.heartbeat_period` | 20 ms (writer) | A `RELIABLE` writer blocked on a full reader history re-syncs only on its periodic heartbeat; the 3 s default exceeds `max_blocking_time` (100 ms) and drops samples under a flat-out publisher with an asynchronous consumer (measured with `bench_e2e`). |
...
**Schema handling.** One provider-owned thread waits on a `WaitSet` over every subscribed topic's `__schema` reader (a 200 ms bounded wait, defensive insurance against a missed wakeup); it decodes the schema, resolves the topic's `SchemaArrival`, and `enable()`s the topic's data reader, which `Subscribe` created **disabled**. Status callbacks for `__schema` endpoints run on that thread; a `SchemaArrival` resolves on it; a `FastDDSStatusListener` callback can run there too (intraprocess matching inside `enable()`), so its threading contract lists that thread as well (see [Statuses](#statuses)). One consequence: time-to-first-row for a remote late joiner grows by one discovery round — the round trip needed to notice the schema arrived.
