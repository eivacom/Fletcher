...
- The callback can also run **on the thread that is still inside `Subscribe` or `CreateTopic`**. `create_datareader` pairs with intraprocess writers synchronously, so a `TRANSIENT_LOCAL` topic that already has a publisher on this same participant replays its retained samples on the subscribing thread — while `Subscribe` is holding the provider mutex. The mirror case is `CreateTopic`: when the `__schema` sample it writes reaches an already-matched reader in this same process synchronously, that reader's schema resolution — `Resolve`, its continuation, and the buffered-row drain — runs on the `CreateTopic` thread too, with *its own* `impl_->mu` held exclusively for the whole call. A callback that re-enters the provider from either delivery would deadlock on the provider mutex — a `std::shared_mutex`, not recursive — but no longer reaches it: since PDA-DEC-AG1 every seam method — the four data-path methods and the two schema-only ones — is refused at a door before any lock, from *any* delivery, first or later. The door prevents that deadlock; what it does not prevent is a stall — every other `Publish` (and every other seam call) on the `CreateTopic`-calling instance waits for the whole chain to finish before the mutex is released. See the re-entrancy bullet below.
