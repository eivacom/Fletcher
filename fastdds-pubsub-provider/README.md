...
- **Schema before data.** The subscription callback is never invoked with a null schema. Because `Subscribe` is non-blocking and may run before any publisher exists, a data sample can arrive before the topic schema does (the schema travels on the separate `__schema` channel). The data reader is enabled first and the arrival resolved only after, so a resolved arrival means the reader is already live; nothing published before that enable has reached the reader to hold, and the built-in `VOLATILE` durability keeps nothing from before either, so a burst published ahead of the match is simply not there to replay, and the callback only ever sees rows after the schema.
...
- The callback can also run **on the thread that is still inside `Subscribe`, or on the schema thread**. `Subscribe` creates the data reader **disabled**; `enable()` runs it — immediately, on the subscribing thread, if this provider already knows the schema, otherwise later, on the schema thread, the moment the `__schema` sample arrives — `enable()` runs there first, and only then does the arrival resolve (see [Delivery guarantees](#delivery-guarantees)). `enable()` pairs with an intraprocess writer synchronously, so a `TRANSIENT_LOCAL` topic that already has a publisher on this same participant replays its retained samples inline, on whichever thread called `enable()` — while that thread holds the provider mutex (`Subscribe`) or `schema_mu` (the schema thread). A callback that re-enters the provider from either delivery would deadlock on the provider mutex — a `std::shared_mutex`, not recursive — but no longer reaches it: every seam method is refused at a door before any lock, from *any* delivery, first or later. The door prevents that deadlock; what it does not prevent is a stall — every other seam call on that instance waits for the whole chain to finish before the mutex is released. See the re-entrancy bullet below.
