...
enum ArrowType WireTypeToNanoarrowType(int wt) {
    switch (wt) {
        ...
        default: {
            ...
            hex << "0x" << std::hex << wt;
            throw std::invalid_argument("create_topic: wireType " + hex.str() +
...
json FieldToJson(const ArrowSchema* field) {
    ...
    if (field->metadata) {
        ...
        if (ArrowMetadataGetValue(field->metadata, ArrowCharView("field_number"), &value) ==
            NANOARROW_OK) {
    ...
    } else if (view.type == NANOARROW_TYPE_LIST || view.type == NANOARROW_TYPE_LARGE_LIST) {
...
OwnedSchema BuildArrowSchemaFromJson(const json& j) {
    if (!j.contains("fields") || !j["fields"].is_array()) {
        throw std::invalid_argument("create_topic: schema must contain a 'fields' array");
    }
    ...
    for (size_t i = 0; i < fields.size(); ++i) {
        ...
        if (!f.contains("name") || !f["name"].is_string()) {
            throw std::invalid_argument("create_topic: field at index " + std::to_string(i) +
